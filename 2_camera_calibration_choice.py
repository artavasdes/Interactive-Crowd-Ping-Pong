...
#os.startfile("Interactive-Crowd-Ping-Pong-main\Interactive_Pong\Build\Pong.exe")
...
#Yes or No Calibration, so that old values can be used
file_path = r'calibration.txt'
callibrate = input("Callibrate light values? (y/n): ")
if (callibrate == "y"):
    #Bright Callibration Image
    leftbright = callibration(LeftCamera, "Left_Bright", True, True)
    cv.imshow("Left Bright", leftbright)
    rightbright = callibration(RightCamera, "Right_Bright", True, False)
    cv.imshow("Right Bright", rightbright)

    #Dark Callibration Image
    leftdark = callibration(LeftCamera, "Left_Dark", False, True)
    cv.imshow("Left Dark", leftdark)
    rightdark = callibration(RightCamera, "Right_Dark", False, False)
    ...
    with open(file_path, 'w') as fp:
        ...
        fp.write(str(round(LeftRealMaxVal,4))+"\n")
        fp.write(str(round(LeftMaxVal,4))+"\n")
        fp.write(str(round(LeftMinVal,4))+"\n")
        fp.write(str(round(RightRealMaxVal,4))+"\n")
        fp.write(str(round(RightMaxVal,4))+"\n")
        fp.write(str(round(RightMinVal,4))+"\n")

else:
    if(os.path.exists(file_path)):
        print('file already exists')
        with open(file_path, 'r') as fp:
            LeftRealMaxVal = float(fp.readline())
            LeftMaxVal = float(fp.readline())
            LeftMinVal = float(fp.readline())
            RightRealMaxVal = float(fp.readline())
            RightMaxVal = float(fp.readline())
            RightMinVal = float(fp.readline())

        print("Starting Values")
        print("Left Values", LeftRealMaxVal, LeftMaxVal, LeftMinVal)
        print("Right Values", RightRealMaxVal, RightMaxVal, RightMinVal)
        #print(type(float(LeftMaxVal)))
